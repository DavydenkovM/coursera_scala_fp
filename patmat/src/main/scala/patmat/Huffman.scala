...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def countChars(acc: Map[Char, Int], char: Char) : Map[Char, Int] = {
      val counter = acc.getOrElse(char, 0) + 1
      acc + (char -> counter)
    }

    chars.foldLeft(Map[Char, Int]())(countChars).toList
  }
